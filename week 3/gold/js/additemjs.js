...
window.addEventListener("DOMContentLoaded", function(){
	...
	function createCategory(){
		...
		createSelect.setAttribute("id","fruitCategory");
		for(var i=0, j=fruitCategories.length; i<j; i++){
			var optionGroup = document.createElement("option");
			var text = fruitCategories[i];
	...
	function controls(x){
		switch(x){
			case "on":
				element("fruittracker").style.display = "none";
				...
				break;
			case "off":
				element("fruittracker").style.display = "block";
	...
	function editItem (){
		...
		element("fruitCategory").value = item.category[1];
		element("fruitName").value = item.fname[1];
	...
	function validateData(){
		...
		var getFruitName = element("fruitName");
		...
		getFruitName.style.backgroundColor= "white";
		getFruitName.style.border = "";
		...
		//Fruit name Validation 
		if (getFruitName.value === ""){
			var snameError = "Please enter fruit name.";
			getFruitName.style.border = "2px solid red";
			getFruitName.style.backgroundColor= "#ffbbbb";
